...
function validateMoodRange(evt){
    ...
    if ((minMood && maxMood)) {
        ...
        overallMood = parseInt(overallMood, radix);
        minMood = parseInt(minMood, radix);
        maxMood = parseInt(maxMood, radix);

        if (minMood > maxMood) {
            msg = '** Min should not be greater than max **';
            // evt.preventDefault();
            // $('.alert-msg').html('** Min should not be greater than max **');
        }
        else if (!((minMood < overallMood) && (overallMood < maxMood))) {
            msg = '** Overall mood should be within mood range **';
            // evt.preventDefault();
            // $('.alert-msg').html('** Overall mood should be within mood range **');
        }
    }
    else if ((!minMood && maxMood) || (!maxMood && minMood)) {
        msg = '** Both min and max must be entered or empty**';
    }
